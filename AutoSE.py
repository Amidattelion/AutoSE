...
# put here the path to your SpaceEngine folder
SE_path = ''

log=open(SE_path+'system/se.log','a')
...
log=open(SE_path+'system/se.log','r')

with open(SE_path+'data/scripts/AutoSE.sc','a') as file:
    file.seek(0)
    file.truncate()
    file.write("Print Lancement d'AutoSE")

with open(SE_path+'data/scripts/AutoSEDEF.sc','a') as file:
    file.seek(0)
    file.truncate()
...
while not stop:
    ...
    shutil.move(SE_path+'export','C:/Users/Adrien/Desktop/AutoSE')
    ...
    lancerScript(creerScript(fname,SE_path+'data/scripts',1))
...
shutil.rmtree(SE_path+'data/scripts/AutoSE.sc')
